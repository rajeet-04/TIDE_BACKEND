...
- ✅ `src/extract_pdfs.py` — PDF extraction with layout parsing
- ✅ `src/constituents.py` — Tidal constituent definitions
- ✅ `src/lsm.py` — LSM harmonic solver
- ✅ `src/evaluate.py` — Metrics computation (RMSE, R, bias, SR)
- ✅ `src/datums.py` — Tidal datum calculation
- ✅ `src/train_predict.py` — LSM pipeline orchestration
- ✅ `src/plot_results.py` — LSM diagnostic plots
- ✅ `src/residual_lstm.py` — LSTM training (Phase 6)
- ✅ `src/plot_lstm.py` — LSM vs LSM+LSTM comparison plots
- ✅ `src/forecast.py` — LSM-only forecasting CLI
- ✅ `src/forecast_combined.py` — LSM+LSTM forecasting CLI
- ✅ `src/validate_forecast.py` — Validation tool
- ✅ `src/plot_validation.py` — Validation visualization
